...
const ThemeToggle: React.FC = () => {
  ...
  useEffect(() => {
    if (isDark) {
      ...
      document.body.classList.replace('text-slate-900', 'text-slate-50');
    } else {
      document.documentElement.classList.remove('dark');
      document.body.classList.replace('bg-slate-900', 'bg-slate-50');
      document.body.classList.replace('text-slate-50', 'text-slate-900');
    }
  }, [isDark]);
  ...
  return (
    <button
      ...
      className="p-2 rounded-full glass hover:bg-slate-200 dark:hover:bg-slate-800 transition-all"
