...
const Projects: React.FC = () => {
  return (
    <section id="projects" className="py-24 px-6 bg-slate-100/50 dark:bg-slate-800/20 scroll-mt-24">
      <div className="max-w-[1600px] mx-auto space-y-16">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {PROJECTS.map(project => (
            <div key={project.id} className="group bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-200 dark:border-slate-800 flex flex-col">
              ...
              <div className="p-8 space-y-6 flex-1 flex flex-col">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.techStack.map(tech => (
                      <span key={tech} className="text-[10px] font-bold bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full text-slate-500 uppercase tracking-wider">{tech}</span>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4 text-sm leading-relaxed flex-1">
                  <div className="space-y-1">
                    ...
                    <p className="text-slate-600 dark:text-slate-300 italic">"{project.problem}"</p>
                  </div>
                  <div className="space-y-1">
                ...
                <div className="flex items-center space-x-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                  <a
                    ...
                    className="flex-1 text-center py-3 rounded-2xl border border-slate-200 dark:border-slate-700 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex items-center justify-center space-x-2"
                  >
